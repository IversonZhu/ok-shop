package com.okman.shop.gateway.filter;
...
import com.okman.shop.common.exception.ValidateCodeException;
import com.okman.shop.common.model.Result;
import com.okman.shop.common.util.WebUtils;
import com.okman.shop.gateway.config.GatewayConfigProperties;
...
public class ValidateCodeGatewayFilter extends AbstractGatewayFilterFactory {
    ...
    public GatewayFilter apply(Object config) {
        return (exchange, chain) -> {
            ...
            // 不是登录请求，直接向下执行
            if (!StrUtil.containsAnyIgnoreCase(request.getURI().getPath(), SysConstants.OAUTH_TOKEN_URL)) {
                return chain.filter(exchange);
            }

            // 刷新token，直接向下执行
            String grantType = request.getQueryParams().getFirst("grant_type");
            if (StrUtil.equals(SysConstants.REFRESH_TOKEN, grantType)) {
                return chain.filter(exchange);
            }
